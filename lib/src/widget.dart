...
class DrivenButton extends StatelessWidget {
  const DrivenButton({
    Key? key,
    required this.label,
    this.leading,
    ...
    this.duration = DrivenButton.defaultDuration,
  }) : super(key: key);
  ...
  /// by [DrivenButtonStyle.driven], then [DrivenButtonStyle.evaluate]
  /// is used for the following [DrivenButtonEvent]s:
  ///
  ///  * [DrivenButtonEvent.disabled].
  ///  * [DrivenButtonEvent.selected].
  ///  * [DrivenButtonEvent.hovered].
  ///  * [DrivenButtonEvent.focused].
  ///  * [DrivenButtonEvent.pressed].
  final DrivenButtonStyle? style;

  /// Used by widgets that expose their internal event
  /// for the sake of extensions that add support for additional events.
  final DrivenButtonEventController? eventsController;
...
/// Chip widget with smooth animation, event driven style, and many more.
class _DrivenButtonRender extends ImplicitlyAnimatedWidget {
  const _DrivenButtonRender({
    Key? key,
    required this.label,
    this.leading,
    this.trailing,
    ...
    Duration duration = DrivenButton.defaultDuration,
  }) : super(
          key: key,
          duration: duration,
          curve: curve,
  ...
  @override
  _DrivenButtonRenderState createState() => _DrivenButtonRenderState();
}

class _DrivenButtonRenderState
    extends AnimatedWidgetBaseState<_DrivenButtonRender>
    with WidgetEventMixin<_DrivenButtonRender> {
  DrivenButtonStyle style = const DrivenButtonStyle();

  @protected
  void setStyle() {
    final style = widget.style ?? DrivenButtonStyle.toned();
    final resolved = DrivenButtonStyle.evaluate(style, widgetEvents.value);
    final result = DrivenButtonStyle.from(resolved);
    this.style = result;
    setState(() {});
  }
  ...
  Color get defaultForegroundColor {
    return style.isFilled
        ? DrivenButtonStyle.colorOnSurface(backgroundColor)!
        : widget.theme.colorScheme.onSurface;
  }

  Color get backgroundColor {
    Color color = style.backgroundColor ?? defaultBackgroundColor;
    color = DrivenButtonStyle.colorWithOpacity(color, style.backgroundOpacity);
    color = DrivenButtonStyle.colorWithAlpha(color, style.backgroundAlpha);
    return color;
  }

  Color get borderColor {
    Color color = style.borderColor ?? defaultBorderColor;
    color = DrivenButtonStyle.colorWithOpacity(color, style.borderOpacity);
    color = DrivenButtonStyle.colorWithAlpha(color, style.borderAlpha);
    return color;
  }

  Color get foregroundColor {
    Color color = style.foregroundColor ?? defaultForegroundColor;
    color = DrivenButtonStyle.colorWithOpacity(color, style.foregroundOpacity);
    color = DrivenButtonStyle.colorWithAlpha(color, style.foregroundAlpha);
    return color;
  }

  Clip get containerClipBehavior {
    return style.clipBehavior ?? DrivenButtonStyle.defaultClipBehavior;
  }

  EdgeInsetsGeometry get containerPadding {
    const defaultPadding = DrivenButtonStyle.defaultPadding;
  ...
  EdgeInsetsGeometry get containerMargin {
    return style.margin ?? DrivenButtonStyle.defaultMargin;
  }

  BorderRadiusGeometry get containerRadius {
    return style.borderRadius ?? DrivenButtonStyle.defaultBorderRadius;
  }
  ...
  ShapeBorder get containerBorder {
    return RoundedRectangleBorder(
      ...
      side: BorderSide(
        color: borderColor,
        width: style.borderWidth ?? DrivenButtonStyle.defaultBorderWidth,
        style: style.borderStyle ?? DrivenButtonStyle.defaultBorderStyle,
      ),
    );
  }
  ...
  double get animatedContainerHeight {
    return _containerHeightTween?.evaluate(animation) ??
        DrivenButtonStyle.defaultHeight;
  }
  ...
  double get animatedForegroundSpacing {
    return _foregroundSpacingTween?.evaluate(animation) ??
        DrivenButtonStyle.defaultForegroundSpacing;
  }
  ...
  double get animatedIconSize {
    return _iconSizeTween?.evaluate(animation) ??
        DrivenButtonStyle.defaultIconSize;
  }
  ...
  void onTap() {
    widgetEvents.toggle(DrivenButtonEvent.pressed, false);
  ...
  void onTapDown(TapDownDetails details) {
    widgetEvents.toggle(DrivenButtonEvent.pressed, true);
  }

  void onHover(bool value) {
    widgetEvents.toggle(DrivenButtonEvent.hovered, value);
  }

  void onFocus(bool value) {
    widgetEvents.toggle(DrivenButtonEvent.focused, value);
  }

  @override
  void initState() {
    initWidgetEvents(widget.eventsController);
    widgetEvents.toggle(DrivenButtonEvent.disabled, widget.disabled);
    setStyle();
    super.initState();
  }
  ...
  @override
  void didUpdateWidget(_DrivenButtonRender oldWidget) {
    if (mounted) {
  ...
  void forEachTween(visitor) {
    _containerHeightTween = visitor(
      ...
      style.height ?? DrivenButtonStyle.defaultHeight,
    ...
    _foregroundSpacingTween = visitor(
      ...
      style.foregroundSpacing ?? DrivenButtonStyle.defaultForegroundSpacing,
      (value) => Tween<double>(begin: value),
    ) as Tween<double>?;
    ...
    _iconSizeTween = visitor(
      _iconSizeTween,
      style.iconSize ?? DrivenButtonStyle.defaultIconSize,
      (value) => Tween<double>(begin: value),
    ) as Tween<double>?;
